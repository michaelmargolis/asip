...
const char ACCELEROMETER_SERVICE = 'A';
// methods
const char  ACCELEROMETER_REQUEST = 'R';   // enable auto events
const char  ACCELEROMETER_MEASURE = 'M';   // measure and send a single event 
// events
const char  ACCELEROMETER_EVENT =   'e';

// Heading (Magnetometer) service 
const char HEADING_SERVICE = 'H';
// methods
const char  HEADING_REQUEST = 'R';   // enable auto events
const char  HEADING_MEASURE = 'M';   // measure and send a single event 
// events
const char  HEADING_EVENT =   'e';

// Pressure (Altitude) service 
const char PRESSURE_SERVICE = 'P';
// methods
const char  PRESSURE_REQUEST = 'R';   // enable auto events
const char  PRESSURE_MEASURE = 'M';   // measure and send a single event 
// events
const char  PRESSURE_EVENT =   'e';
...
public:
   ...
   gyroClass(const char svcId, const char evtId);  
...
public:
   ...
   AccelerometerClass(const char svcId, const char evtId);  
   void begin(byte nbrElements);   // classes that use I2C instead of specific pins use this begin method
   void reportValue(int sequenceId, Stream * stream) ; // send the value of the given device
...
public:
   ...
   HeadingClass(const char svcId, const char evtId);  
   void begin(byte nbrElements);   // classes that use I2C instead of specific pins use this begin method
   void reportValue(int sequenceId, Stream * stream) ; // send the value of the given device
   void reportValues(Stream *stream); // send all values separated by commas, preceded by header and terminated with newline  
   void processRequestMsg(Stream *stream);
...
public:
   ...
   PressureClass(const char svcId, const char evtId);  
   void begin(byte nbrElements);   // classes that use I2C instead of specific pins use this begin method
   void reportValue(int sequenceId, Stream * stream) ; // send the value of the given device
   void processRequestMsg(Stream *stream);
private:
